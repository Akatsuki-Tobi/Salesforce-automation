...
export class PlaywrightExecutor {
  ...
  async navigate(url: string): Promise<void> {
    if (!this.page) throw new Error("No Playwright page available.");
  ...
  async executeAction(plan: PlannerOutput): Promise<void> {
    if (!this.page) throw new Error("No Playwright page available.");
    const { action, params } = plan;

    switch (action) {
      case "navigate":
        if (typeof params.url !== "string") throw new Error("navigate action missing url");
        await this.navigate(params.url);
        break;
      case "click":
        if (typeof params.selector !== "string") throw new Error("click action missing selector");
      ...
      case "type":
        ...
        break;
      case "hover":
        if (typeof params.selector !== "string") throw new Error("hover action missing selector");
