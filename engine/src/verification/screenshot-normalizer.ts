...
export class ScreenshotNormalizer {
   ...
   */
  async normalize(screenshotBuffer: Buffer): Promise<NormalizedScreenshot> {
    ...
    // Convert to buffer
    const outputBuffer =
      this.options.outputFormat === "jpeg"
   ...
   */
  private normalizeTheme(ctx: CanvasRenderingContext2D, width: number, height: number): void {
   ...
   */
  private removeAnimationArtifacts(ctx: CanvasRenderingContext2D, width: number, height: number): void {
   ...
   */
  private normalizeFonts(ctx: CanvasRenderingContext2D, width: number, height: number): void {
   ...
   */
  private removeRandomIdArtifacts(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    const imageData = ctx.getImageData(0, 0, width, height);
    const data = imageData.data;
  ...
  async normalizePair(
    screenshotA: Buffer,
    screenshotB: Buffer
  ): Promise<{ normalizedA: NormalizedScreenshot; normalizedB: NormalizedScreenshot }> {
   ...
   */
  async saveToFile(normalizedScreenshot: NormalizedScreenshot, filePath: string): Promise<void> {
