...
export class FeatureMatcher {
  private options: FeatureMatchOptions;
   ...
   */
  async compare(screenshotA: Buffer, screenshotB: Buffer, threshold: number = 0.6): Promise<FeatureMatchResult> {
  ...
  private extractKeypoints(image: any, width: number, height: number): Keypoint[] {
  ...
  private computeDescriptor(gray: Float64Array, x: number, y: number, width: number, height: number): Float32Array {
  ...
  private matchKeypoints(keypointsA: Keypoint[], keypointsB: Keypoint[]): Array<{ a: Keypoint; b: Keypoint; distance: number }> {
    const matches: Array<{ a: Keypoint; b: Keypoint; distance: number }> = [];

    for (const kpA of keypointsA) {
      let bestMatch: { b: Keypoint; distance: number } | null = null;
  ...
  private calculateDistance(descA: Float32Array, descB: Float32Array): number {
    let distance = 0;
