...
class PixelComparator {
    ...
    isIgnoredRegion(x, y) {
        if (!this.options.ignoreRegions)
            return false;
        return this.options.ignoreRegions.some((region) => x >= region.x && x < region.x + region.width && y >= region.y && y < region.y + region.height);
    }
    findBoundingBoxes(diffMask, width, height) {
        const boxes = [];
    ...
    floodFill(diffMask, visited, startX, startY, width, height) {
