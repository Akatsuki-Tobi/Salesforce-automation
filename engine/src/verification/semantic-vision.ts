...
export interface SemanticComparisonResult {
  ...
  intentMatch: number;
  elementMatches: Array<{ expected: string; actual: string; similarity: number }>;
  details: {
...
export class SemanticVision {
  private options: SemanticVisionOptions;
   ...
   */
  async describeScreenshot(screenshot: Buffer): Promise<SemanticDescription> {
  ...
  private detectSemanticElements(ctx: CanvasRenderingContext2D, width: number, height: number): Array<{ type: string; description: string; position: { x: number; y: number }; confidence: number }> {
    const elements: Array<{ type: string; description: string; position: { x: number; y: number }; confidence: number }> = [];
  ...
  private analyzeLayout(ctx: CanvasRenderingContext2D, width: number, height: number): string {
    ...
    if (hasHeader && hasSidebar && hasFooter) return "header-sidebar-footer";
    if (hasHeader && hasFooter) return "header-footer";
    if (hasSidebar) return "sidebar";
    return "standard";
  }

  private analyzeColorScheme(ctx: CanvasRenderingContext2D, width: number, height: number): string {
    const imageData = ctx.getImageData(0, 0, width, height);
    const data = imageData.data;
  ...
  private calculateSemanticSimilarity(descA: SemanticDescription, descB: SemanticDescription): number {
    // Compare layouts
  ...
  private calculateIntentMatch(description: SemanticDescription, expectedIntent: string): number {
  ...
  private compareElements(
    elementsA: Array<{ type: string; description: string; position: { x: number; y: number }; confidence: number }>,
    elementsB: Array<{ type: string; description: string; position: { x: number; y: number }; confidence: number }>
  ): Array<{ expected: string; actual: string; similarity: number }> {
    const matches: Array<{ expected: string; actual: string; similarity: number }> = [];

    for (const elemA of elementsA) {
      let bestMatch: { actual: string; similarity: number } | null = null;
  ...
  private findButtonRegions(data: Uint8ClampedArray, width: number, height: number): Array<{ x: number; y: number }> {
  ...
  private isButtonRegion(data: Uint8ClampedArray, x: number, y: number, width: number, height: number): boolean {
  ...
  private findInputRegions(data: Uint8ClampedArray, width: number, height: number): Array<{ x: number; y: number }> {
    const regions: Array<{ x: number; y: number }> = [];
  ...
  private isInputRegion(data: Uint8ClampedArray, x: number, y: number, width: number, height: number): boolean {
  ...
  private findImageRegions(data: Uint8ClampedArray, width: number, height: number): Array<{ x: number; y: number }> {
    const regions: Array<{ x: number; y: number }> = [];
  ...
  private isImageRegion(data: Uint8ClampedArray, x: number, y: number, width: number, height: number): boolean {
  ...
  private detectHeader(data: Uint8ClampedArray, width: number, height: number): boolean {
  ...
  private detectSidebar(data: Uint8ClampedArray, width: number, height: number): boolean {
  ...
  private detectFooter(data: Uint8ClampedArray, width: number, height: number): boolean {
