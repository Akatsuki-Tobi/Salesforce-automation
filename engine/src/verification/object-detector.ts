...
export interface ObjectVerificationResult {
  ...
  passed: boolean;
  threshold: number;
  matchedObjects: Array<{ expected: DetectedObject; actual: DetectedObject; similarity: number }>;
  missingObjects: DetectedObject[];
  extraObjects: DetectedObject[];
  positionChanges: Array<{ object: string; expectedPos: { x: number; y: number }; actualPos: { x: number; y: number }; distance: number }>;
  details: {
...
export class ObjectDetector {
  private objectTypes = ["button", "textbox", "dropdown", "image", "icon", "checkbox", "modal", "link", "label", "table"];

  /**
   * Detect objects in a screenshot
   * Note: In production, this would use a trained ML model like YOLO or DETR
   */
  async detectObjects(screenshot: Buffer): Promise<ObjectDetectionResult> {
   ...
   */
  async verifyObjects(screenshot: Buffer, expectedObjects: DetectedObject[], threshold: number = 0.8): Promise<ObjectVerificationResult> {
    const detectionResult = await this.detectObjects(screenshot);
    const actualObjects = detectionResult.objects;

    const matchedObjects: Array<{ expected: DetectedObject; actual: DetectedObject; similarity: number }> = [];
    const missingObjects: DetectedObject[] = [];
    const extraObjects: DetectedObject[] = [];
    const positionChanges: Array<{ object: string; expectedPos: { x: number; y: number }; actualPos: { x: number; y: number }; distance: number }> = [];
  ...
  private detectObjectsHeuristic(data: Uint8ClampedArray, width: number, height: number): DetectedObject[] {
  ...
  private detectRectangularRegions(data: Uint8ClampedArray, width: number, height: number, constraints: { minWidth: number; minHeight: number; maxWidth: number; maxHeight: number }): Array<{ x: number; y: number; width: number; height: number }> {
  ...
  private findRectangularRegion(data: Uint8ClampedArray, startX: number, startY: number, width: number, height: number, constraints: { minWidth: number; minHeight: number; maxWidth: number; maxHeight: number }): { x: number; y: number; width: number; height: number } | null {
 ...
 MEA  private isRegionStart(data: Uint8ClampedArray, x: number, y: number, width: number): boolean {
  ...
  private isEdge(data: Uint8ClampedArray, x: number, y: number, width: number, direction: string): boolean {
    const idx = (y * width + x) * 4;
    const brightness = (data[idx] + data[idx + 1] + data[idx + 2]) / 3;
    return brightness < 180; // Edge pixel
  }

  private detectImageRegions(data: Uint8ClampedArray, width: number, height: number): Array<{ x: number; y: number; width: number; height: number }> {
    const regions: Array<{ x: number; y: number; width: number; height: number }> = [];
  ...
  private calculateColorVariation(data: Uint8ClampedArray, startX: number, startY: number, size: number, width: number, height: number): number {
  ...
  private buildHierarchy(objects: DetectedObject[]): Array<{ parent: string; children: string[] }> {
  ...
  private isContained(inner: { x: number; y: number; width: number; height: number }, outer: { x: number; y: number; width: number; height: number }): boolean {
    return inner.x >= outer.x && inner.y >= outer.y && inner.x + inner.width <= outer.x + outer.width && inner.y + inner.height <= outer.y + outer.height;
  }

  private calculateObjectSimilarity(objA: DetectedObject, objB: DetectedObject): number {
  ...
  private calculateDistance(bboxA: { x: number; y: number; width: number; height: number }, bboxB: { x: number; y: number; width: number; height: number }): number {
